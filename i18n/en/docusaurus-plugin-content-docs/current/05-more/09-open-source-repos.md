...
macOS GUI operations still run through HostAgent. To give Windows users local GUI automation before the native HostAgent is complete, Windows temporarily uses the independent third-party CUA Driver bundled with DesireCore. This does not mean HostAgent has been deprecated or replaced. Third-party components are not DesireCore official repositories; see [Third-Party Software and Licenses](./10-third-party-software.md).
:::
