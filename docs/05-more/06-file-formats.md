...
| 格式 | 读取支持 | 导出支持 | 说明 |
|------|----------|----------|------|
| Markdown (`.md`) | 是 | 是 | 聊天、文档和 Skill 的主要文本格式 |
| PDF (`.pdf`) | 是 | 是 | 支持文本提取、按页视觉读取、长 PDF 分段读取 |
| Word (`.docx`) | 是 | 是 | Office 类技能可读写，SuperDoc 可导出 |
| Excel / CSV | 是 | 视技能而定 | 数据分析和表格处理常用 |
| PowerPoint | 是 | 视技能而定 | 演示文稿技能可处理 |
| 图片 | 是 | 是 | 支持视觉理解、对话嵌入、图像生成 |
| draw.io (`.drawio`) | 是 | 是 | 图表可在本地 draw.io 编辑器中打开 |
| ZIP | 是 | 是 | Agent、Team、Skill 导入导出 |

PDF 读取会根据文件大小和内容选择策略：文字页优先文本提取，扫描页或图表页可渲染为图片交给视觉模型；大文件会按范围读取，避免一次性加载过多内容。
