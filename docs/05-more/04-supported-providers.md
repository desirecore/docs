...
DesireCore 的供应商和模型列表由 config-center 持续更新，实际可用项会随客户端、账号权限和远端配置变化。本文列出常见供应商和能力类型，作为配置参考；具体模型、价格和上下文长度请以「设置 > 算力服务」中的实时列表为准。

支持的服务类型包括对话、推理、工具调用、视觉理解、嵌入、语音合成、语音识别、图像生成、视频生成、音乐生成、OCR、Computer Use 等。

:::tip 生成类模型
`image_gen`、`video_gen`、`music_gen`、`tts`、`asr` 等模型不会作为普通聊天模型出现。它们由对应工具或技能调用。
:::
