...
为了统计安装量和平台分布，打包版 DesireCore 的默认实例会向发布服务发送安装统计。成功发送后，同一 UTC 日不再重复发送；网络失败时，应用可能在后续检查中重试。该信息仅包含：由硬件标识经 SHA-256 生成的去标识化设备标识、客户端版本号、操作系统平台和 CPU 架构。设备标识不包含姓名、邮箱或对话内容，但可用于识别同一安装，因此不应被理解为完全匿名的数据。

为了了解产品功能是否有效，DesireCore 还会按 UTC 日在本地汇总使用统计，并在次日或恢复联网后发送。报告包含：与硬件无关的随机统计标识（UUID）、汇总日期、客户端版本、操作系统平台，以及任务数、执行轮次、成功/失败/部分完成次数、多智能体任务数、用户中止、后台超时、任务验收、文件交付、消息发送和去重智能体数量等纯数字计数。报告不包含对话内容、提示词、产物、智能体名称或标识、任务标题、用户名、邮箱、文件名或文件路径。用于计算去重智能体数量的标识只在本地保存，出网时只发送数量。

安装统计的设备标识与产品使用统计的随机标识彼此独立，不会放在同一请求中，也不会互相派生。当前版本的「发送使用统计」默认为开启，同时控制两类统计。你可以在 **设置 → 数据与隐私** 中随时关闭；关闭后应用立即停止发送和本地累加产品使用统计，删除本地随机统计标识及待发队列。重新开启会生成新的随机统计标识。关闭不会自动删除此前已经发送的记录。
