...
「发送使用统计」默认开启。启用时，打包版 DesireCore 的默认实例会为统计安装量和平台分布向发布服务发送安装统计。成功发送后，同一 UTC 日不再重复发送；网络失败时，应用可能在后续检查中重试。该信息仅包含：由硬件标识经 SHA-256 生成的去标识化设备标识、客户端版本号、操作系统平台和 CPU 架构。设备标识不包含姓名、邮箱或对话内容，但可用于识别同一安装，因此不应被理解为完全匿名的数据。

启用时，为了了解产品功能是否有效，DesireCore 还会按 UTC 日在本地汇总使用统计，并在次日或恢复联网后发送。报告包含：与硬件无关的随机统计标识（UUID）、汇总日期、客户端版本、操作系统平台，以及任务数、执行轮次、成功/失败/部分完成次数、多智能体任务数、用户中止、后台超时、任务验收、文件交付、消息发送和去重智能体数量等纯数字计数。报告不包含用户名、邮箱、账户资料、对话、提示词、模型输出、产物、文件名、文件内容、文件路径、智能体名称或标识、任务标题。DesireCore 不会为了生成统计报告而读取这些用户数据。用于计算去重智能体数量的标识只在本地保存，出网时只发送数量。

安装统计的设备标识与产品使用统计的随机标识彼此独立，不会放在同一请求中，也不会互相派生。「发送使用统计」同时控制两类统计，并在当前版本中默认开启；你可以随时通过 **设置 → 数据与隐私** 主动关闭。关闭后应用立即停止发送和本地累加产品使用统计，删除本地随机统计标识及待发队列。重新开启会生成新的随机统计标识。关闭不会自动删除此前已经发送的记录。
...
本隐私政策采用版本化管理。每次更新都会创建新的版本；DesireCore 会在进入主界面前展示当时生效的隐私政策与使用条款，并要求用户再次明确同意。应用和官网构建只包含当前生效正文，不提供历史版本查看功能。仅继续使用软件不视为接受新版本。用户不同意时可以停止使用；关闭非必要的「发送使用统计」不会影响核心功能。
