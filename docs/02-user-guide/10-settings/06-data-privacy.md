...
**设置 → 数据与隐私 → 发送使用统计** 是安装统计与产品使用统计的统一开关，当前版本默认关闭。只有你主动开启后，DesireCore 才会开始累加和发送以下统计。
...
两类标识互不派生，也不会出现在同一请求中。产品统计会先在本地按日汇总，次日或恢复联网后发送；未发送队列最多保留 30 天。用于计算“使用过的 Agent 数量”的 Agent 标识只保存在本地，出网时只发送数量。
...
- [数据安全](../11-security/04-data-security.md)：了解数据保护架构和 API Key 安全存储
- [审计跟踪](../11-security/03-audit-trail.md)：查看接口审计和导出审计日志
- [用户档案](./07-user-profile.md)：管理个人信息和画像数据
