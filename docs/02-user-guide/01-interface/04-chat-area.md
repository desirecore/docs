...
| **更多** | 展开更多操作（聊天记录、导出、多选、上下文压缩、Rewind、清除历史、删除智能体等） |
...
- **问答卡片**：智能体需要你补充信息或做选择时展示
- **审批卡片**：高风险操作执行前请求你的确认
- **任务板**：长任务拆分后的子任务、状态和进度
- **图表预览**：Mermaid、draw.io 等图表的预览和全屏查看
...
- 将图片**拖拽**到输入区域
- 将文件或文件夹拖到整个聊天区，高亮后松手添加为附件或工作目录引用
...
悬停消息时会显示操作栏。窄屏下操作栏会收起为图标，避免文字逐字换行。

消息旁的时间默认使用简短格式；将鼠标悬停在时间上，可以查看包含日期和秒数的完整本地时间，便于核对跨天或密集执行记录。

常见操作包括：

- 复制纯文本或复制 Markdown
- 重试上一轮回复
- 编辑用户消息，包括增删图片附件，以及删除或重新选择原消息的文档引用
- 朗读消息
- 多选消息并导出
- 从某条用户消息前执行 Rewind
