...
2. **手动导入** --- 在技能管理中点击「导入技能」，选择 `.md` 技能文件、包含 `SKILL.md` 的文件夹，或 `.zip` 技能包

全局技能存储在 `~/.desirecore/skills/` 目录下，智能体专属技能存储在对应 Companion 仓库内。导入失败时，界面会显示具体错误原因，例如缺少 `SKILL.md`、路径不可访问或压缩包结构不正确。
...
项目目录中的 `.agents/skills/` 或 `.claude/skills/` 也可以提供项目级技能。项目级技能只在对应工作目录上下文中可用。
...
如果还需要调整默认模型，请编辑 `agent.json` 的 `llm` 字段；旧的 `runtime` 模型配置会被兼容迁移，但新配置应写入 `llm`。
