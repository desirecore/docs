...
统计不涉及用户本身的数据：不会读取或发送用户名、邮箱、账户资料、对话、提示词、模型输出、产物、文件名、文件内容、路径、Agent 名称或标识、任务标题。关闭后会停止发送和本地累加产品统计，并删除本地随机统计标识和待发队列。详见[隐私政策](../privacy.md)和[数据与隐私设置](../02-user-guide/10-settings/06-data-privacy.md)。
