...
使用 DesireCore 官方云算力（DesireCore Cloud）时，你的对话内容经由 DesireCore 运营的 new-api 网关转发至上游模型供应商。网关仅做请求路由和鉴权，不持久化对话内容。

如果你使用第三方中转网关（如自建 NewAPI），数据则经过该网关运营者的服务器——其安全由运营者负责。

:::tip
对数据路径有严格要求时，可直连模型供应商 API（如 OpenAI、Anthropic），或使用 Ollama 本地部署，数据完全不离开你的设备。
:::
